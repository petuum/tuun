...
class AcqOptimizer:
    ...
    def set_params(self, params):
        ...
        self.params.domain_params = getattr(params, 'domain_params', None)
    ...
    def set_domain(self, domain):
        ...
        if domain is None:
            # Set domain to default params or using AcqOptimizer params

            if not self.params.domain_params:
                # TODO: base next line off of the data, instead of just setting to
                # a 1d domain with arbitrary range
                domain = Namespace(dom_str='real', min_max=[(-10, 10)])
            else:
                domain = dict_to_namespace(self.params.domain_params)

        if isinstance(domain, Namespace):
            ...
            # If not None, Namespace, or dict, treat as Domain instance
            self.domain = domain

        # Set self.params.domain_params to the chosen domain's params
        self.params.domain_params = getattr(self.domain, 'params', None)
    ...
    def optimize(self, acqfunction, data=None):
        ...
        # Carry out acquisition optimization
